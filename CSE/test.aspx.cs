...
public partial class Default2 : System.Web.UI.Page
{
    ...
    {
        conn.Open();
        string sql = "INSERT INTO project (ProjName_TH,ProjName_ENG,Member,Status_ID) OUTPUT INSERTED.ProjiD VALUES ('" + text_th.Text + "','" + text_en.Text + "','" + memberControl.member + "','" + "0" + "')";
        SqlCommand com = new SqlCommand(sql, conn);
    ...
    {
        conn.Open();
        string sql = "UPDATE project SET ProjName_TH = '" + text_th.Text + "'," + " ProjName_ENG = '" + text_en.Text + "' " + " WHERE ProjID = '" + ProjID + "'";
        SqlCommand com = new SqlCommand(sql, conn);
        com.ExecuteScalar();
        conn.Close();
    }
    ...
    protected void btn_cancelSentForm_Click(object sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show("อยู่ใน ปุ่มยกเลิก","ทดสอบคำสั่ง");
            ...
            if (youAreStudent(id, conn)) // นิสิต
            {
                if (haveProject(id, conn)) // มีโปรเจคแล้ว
                {
                    ...
                    if(findStatusProj(ProjID,conn)==1)
                    {
                        ...
                        btn_cancelForm.CssClass = "btn btn-danger disabled";
                        btn_saveForm.CssClass = "btn btn-primary disabled";
                        btn_sentForm.Enabled = false;
                        divAddMember.Style.Add("display", "none");
                        /***************************************/
